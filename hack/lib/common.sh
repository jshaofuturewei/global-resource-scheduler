...
function kube::common::start_apiserver()  {
    ...
    ${CONTROLPLANE_SUDO} rm -f  "${PARTITION_CONFIG_DIR}apiserver.config"
    ${CONTROLPLANE_SUDO} cp hack/apiserver.config "${PARTITION_CONFIG_DIR}apiserver.config"
    echo "Created apiserver partition config file  ${PARTITION_CONFIG_DIR}apiserver.config..."

    ${CONTROLPLANE_SUDO}  sed -i "s/tenant_begin,tenant_end/${partition_begin},${partition_end}/gi"  ${PARTITION_CONFIG_DIR}apiserver.config
    security_admission=""
...
EOF
    ...
    ${CONTROLPLANE_SUDO} "${GO_OUT}/hyperkube" kube-apiserver "${authorizer_arg}" "${priv_arg}" ${runtime_config} \
      ...
      --partition-config="${PARTITION_CONFIG_DIR}apiserver.config" \
