...
import (
	...
	klog "k8s.io/klog"
	scheme "k8s.io/kubernetes/globalscheduler/pkg/apis/allocation/client/clientset/versioned/scheme"
...
type AllocationsGetter interface {
	...
	AllocationsWithMultiTenancy(namespace string, tenant string) AllocationInterface
}
...
func newAllocations(c *AllocationV1Client, namespace string) *allocations {
	return newAllocationsWithMultiTenancy(c, namespace, "system")
}

func newAllocationsWithMultiTenancy(c *AllocationV1Client, namespace string, tenant string) *allocations {
	return &allocations{
		client:  c.RESTClient(),
...
func (c *allocations) List(opts metav1.ListOptions) (result *v1.AllocationList, err error) {
	...
	if wgLen > 1 {
		...
		wg.Add(wgLen)
		results := make(map[int]*v1.AllocationList)
		errs := make(map[int]error)
		for i, client := range c.clients {
			go func(c *allocations, ci rest.Interface, opts metav1.ListOptions, lock *sync.Mutex, pos int, resultMap map[int]*v1.AllocationList, errMap map[int]error) {
				r := &v1.AllocationList{}
				err := ci.Get().
		...
		itemsMap := make(map[string]*v1.Allocation)
		for j := 0; j < wgLen; j++ {
...
func (c *allocations) Watch(opts metav1.ListOptions) watch.AggregatedWatchInterface {
	var timeout time.Duration
	if opts.TimeoutSeconds != nil {
		timeout = time.Duration(*opts.TimeoutSeconds) * time.Second
	}
...
func (c *allocations) Create(allocation *v1.Allocation) (result *v1.Allocation, err error) {
	...
	objectTenant := allocation.ObjectMeta.Tenant
	if objectTenant == "" {
		objectTenant = c.te
	}
